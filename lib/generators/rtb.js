...
function createRtbExport (challenges, { insertHints, insertHintUrls, ctfKey, juiceShopUrl }) {
  ...
  function insertHint(challenge, flag) {
    if (checkHints || checkHintsURL) {
      ...
      if (checkHintsURL) {
        const hint = hints.ele("hint")
        hint.ele("description", "[" + formatHintURL(challenge.hintUrl) + 
          "](" + challenge.hintUrl + ")")
  ...
  function formatHintURL(url) {
    url = url.substring(url.lastIndexOf("#")+1, url.length).replace(/-/g, " ")
    url = url.replace(/\b[a-z]|['_][a-z]|\B[A-Z]/g, function(x){
