    ...
    try {
      ...
      for (var key in challenges) {
        if (challenges.hasOwnProperty(key)) {
          ...
          sql = sql + '0);\n'
          if (challenge.hint && insertHints !== answers.noTextHints) {
            sql = sql + 'INSERT INTO hints (id, chal, hint, cost, type) VALUES ('
            sql = sql + '' + challenge.id + ', '
            sql = sql + '' + challenge.id + ', '
            sql = sql + '"' + challenge.hint.replace(/"/g, '""') + '", '
            sql = sql + '' + (insertHints === answers.paidTextHints ? score / 10 : 0) + ', '
            sql = sql + '0);\n'
          }
          if (challenge.hintUrl && insertHintUrls !== answers.noHintUrls) {
            sql = sql + 'INSERT INTO hints (id, chal, hint, cost, type) VALUES ('
            sql = sql + '' + (10000 + challenge.id) + ', '
            sql = sql + '' + challenge.id + ', '
            sql = sql + '"' + challenge.hintUrl + '", '
            sql = sql + '' + (insertHintUrls === answers.paidHintUrls ? score / 5 : 0) + ', '
            sql = sql + '0);\n'
          }
          sql = sql + '\r'
        }
